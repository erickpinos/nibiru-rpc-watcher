...
Monitors the Nibiru archive node (`evm-rpc.archive.nibiru.fi`) for uptime, block height stalls, and response time. Logs to Neon and alerts via Telegram.
...
- `TELEGRAM_BOT_TOKEN`
- `TELEGRAM_CHAT_ID`
- `RPC_URL` (optional, defaults to `https://evm-rpc.archive.nibiru.fi`)
...
| `RPC_URL` | No | `https://evm-rpc.archive.nibiru.fi` | Archive node EVM RPC |
